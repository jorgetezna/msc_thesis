...
def main():
    ...
    for i in range(1, len(image_files)):
        ...
        mag = compute_optical_flow(image1, image2)
        mask = cv2.threshold(mag, 1.0, 255, cv2.THRESH_BINARY)[1].astype(np.uint8)
        ...
        # Calculate metrics
        precision, recall, f1, tp, fp, fn, tn = evaluate_performance(clustered_detections, ground_truth_boxes)
        total_tp += tp
        total_fp += fp
        total_fn += fn
        total_tn += tn
