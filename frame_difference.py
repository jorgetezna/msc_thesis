...
def get_mask(frame1, frame2, kernel=np.ones((11, 11), np.uint8)):
    frame_diff = cv2.subtract(frame2, frame1)

    # Blur the frame difference
    frame_diff = cv2.medianBlur(frame_diff, 7)
...
def get_contour_detections(mask, thresh=1):  # Set threshold to 1 for very low threshold
...
def main():
    image_folder = '/Users/jorgemartinez/thesis_retinanet/datasets/west_easy_cars_no_padding_yolo/all/masked'
    image_pairs = [(os.path.join(image_folder, 'image1.jpg'), os.path.join(image_folder, 'image2.jpg'))]  # Add your image pairs here
    ...
    for i in range(0, len(image_files) - 1, 2):
        ...
        mask = get_mask(gray1, gray2)
        detections = get_contour_detections(mask, thresh=1)
        ...
        ap = compute_average_precision(detections, ground_truth_boxes, iou_threshold=0.0)
        all_aps.append(ap)
